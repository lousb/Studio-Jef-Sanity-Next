      ...
      project.content.forEach((item) => {
        ...
        if (_type === 'hybridMedia' && item.featured) {
          ...
          if (video?.playbackId) {
            ...
            extracted.push({ type: 'image', src: image.url, caption: caption || '', key: _key })
          }
        }
        if (_type === 'twoHybridMedia') {
          if (item.leftFeatured) {
            ...
            if (video?.playbackId) {
              ...
              extracted.push({ type: 'image', src: image.url, caption: caption || '', key: `${_key}-left` })
            }
          }
          if (item.rightFeatured) {
            ...
            if (video?.playbackId) {
              ...
              extracted.push({ type: 'image', src: image.url, caption: caption || '', key: `${_key}-right` })
            }
          }
        }
      })
  ...
  const createMediaElement = (item, seed, x, y, spacing) => {
    ...
    if (isVideo && item.playbackId) {
      ...
      const handleReady = () => {
        ...
        const h = 1050 + seededRandom(seed + 4) * 40
        const w = h * ratio
        el.appendChild(img)
        processMedia(w, h)
      }
  ...
  const generateItems = () => {
    ...
    for (let i = 0; i < countX; i++) {
      for (let j = 0; j < countY; j++) {
        ...
        createMediaElement(item, seed, x, y, spacing)
      }
    }
  }

  const render = () => {
    const s = scroll.current, m = mouse.current
